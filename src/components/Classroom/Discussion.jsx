...
// here we start creating our live communication
...
const Discussion = ({ classCode, adminEmail }) => {
  ...
  const getDiscussion = async () => {
    ...
    await axios
      .post(
        "http://localhost:5000/classes/getDiscussions",
        {
          classCode: classCode,
        },
  ...
  socket.on("disscusionmessages", (messgaes) => {
    ...
    if (fileInput) {
      ...
      uploadTask.on("state_changed", console.log, console.error, () => {
        storage
          ...
          .then((firebaseURL) => {
            return axios.post(
              "http://localhost:5000/classes/createDiscussion",
              {
      ...
      // Here we send socket message to the backend ------>
      socket.emit("classdisscussion", {
        _id: "XXXXXX",
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        // This is where we render our chat''
        <div className="Posts">
          {posts.map((post) => {
            return (

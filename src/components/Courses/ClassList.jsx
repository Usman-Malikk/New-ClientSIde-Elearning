...
const ClassList = (props) => {
  ...
  const RenderClasses = () => {
    return owned.concat(enrolled).length == 0 ? (
      <div className="text-muted text-center">
        You haven't created or joined any courses.
      </div>
    ) : (
      owned.concat(enrolled).map((sub, index) => {
        ...
        return (
          ...
          >
            {/* Here we will open the modal for stripe payment */}
            <Link
              to={"/courses/" + sub.classCode}
  ...
  const RenderClassesMobile = () => {
    return (activeTab == "1" ? owned : enrolled).length == 0 ? (
      <div className="text-muted text-center">
        You haven't {activeTab == "1" ? "created" : "joined"} any course.
      </div>
    ) : (
